...
size_t MaxDisplayedMissions(bool onRight) {
  return static_cast<unsigned>(
      max(0, static_cast<int>(
                 floor((Screen::Height() - (onRight ? 160. : 190.)) / 20.))));
}

// Compute the required scroll amount for the given list of jobs/missions.
void ScrollMissionList(const list<Mission> &missionList,
                       const list<Mission>::const_iterator &it,
                       double &sideScroll, bool checkVisibility) {
  // We don't need to scroll at all if the selection must be within the
  // viewport. The current scroll could be non-zero if missions were
  // added/aborted, so return the delta that will reset it.
  ...
  if (missionCount < maxViewable)
    ...
    const auto countBefore = static_cast<size_t>(
        checkVisibility
            ? count_if(missionList.begin(), it,
                       [](const Mission &m) { return m.IsVisible(); })
    ...
    if (desiredScroll < sideScroll) {
      ...
      // Scroll downwards (but not so far that the list's bottom sprite comes
      // upwards further than needed).
      sideScroll =
          min(maximumScroll, sideScroll + (desiredScroll - bottomOfPage));
    }
  }
}
} // namespace
...
MissionPanel::MissionPanel(PlayerInfo &player)
    : MapPanel(player), missionInterface(GameData::Interfaces().Get("mission")),
      available(player.AvailableJobs()), accepted(player.Missions()),
      availableIt(player.AvailableJobs().begin()),
      acceptedIt(player.AvailableJobs().empty() ? accepted.begin()
                                                : accepted.end()),
      tooltip(150, Alignment::LEFT, Tooltip::Direction::DOWN_RIGHT,
              Tooltip::Corner::BOTTOM_LEFT,
...
MissionPanel::MissionPanel(const MapPanel &panel)
    : MapPanel(panel), missionInterface(GameData::Interfaces().Get("mission")),
      available(player.AvailableJobs()), accepted(player.Missions()),
      availableIt(player.AvailableJobs().begin()),
      acceptedIt(player.AvailableJobs().empty() ? accepted.begin()
                                                : accepted.end()),
  ...
  if (!FindMissionForSystem(selectedSystem) &&
      !(player.GetSystem() != selectedSystem &&
        FindMissionForSystem(player.GetSystem())) &&
...
void MissionPanel::Step() {
  ...
  if (GetUI().IsTop(this) && player.GetPlanet() &&
      player.GetDate() >= player.StartData().GetDate() + 12)
    DoHelp("map advanced");
  DoHelp("jobs");
  ...
  if (GetUI().IsTop(this) && !fromMission) {
    ...
    if (mission) {
      ...
      specialSystem =
          mission->IsVisible() ? mission->Destination()->GetSystem() : nullptr;
...
void MissionPanel::Draw() {
  ...
  const double jumpRange =
      flagship ? flagship->JumpNavigation().JumpRange() : 0.;
  ...
  if (availableIt != available.end() && availableIt->Destination())
    DrawMissionSystem(*availableIt,
                      CanAccept() ? availableColor : unavailableColor);
  if (acceptedIt != accepted.end() && acceptedIt->Destination())
    DrawMissionSystem(*acceptedIt,
                      IsSatisfied(*acceptedIt) ? currentColor : blockedColor);

  Point pos;
  if (player.GetPlanet()) {
    pos = DrawPanel(Screen::TopLeft() + Point(0., -availableScroll),
                    "此地可接任务：", available.size(), true);
    DrawList(available, pos, availableIt, true);
  }

  pos = DrawPanel(Screen::TopRight() + Point(-SIDE_WIDTH, -acceptedScroll),
                  "当前任务：", AcceptedVisible());
  DrawList(accepted, pos, acceptedIt);

  // Now that the mission lists and map elements are drawn, draw the top-most UI
  // elements.
  DrawKey();
  DrawMissionInfo();
  DrawTooltips();
...
bool MissionPanel::Click(int x, int y, MouseButton button, int clicks) {
  ...
  if (x < Screen::Left() + SIDE_WIDTH) {
    ...
    // Available missions
    unsigned index = max(
        0, (y + static_cast<int>(availableScroll) - 36 - Screen::Top()) / 20);
    ...
    // Accepted missions
    int index = max(
        0, (y + static_cast<int>(acceptedScroll) - 36 - Screen::Top()) / 20);
  ...
  if (system) {
    ...
    if ((availableIt != available.end() && !Involves(*availableIt, system)) ||
      ...
      auto firstExistingIt =
          find_if(available.begin(), available.end(),
...
bool MissionPanel::Drag(double dx, double dy) {
  if (dragSide < 0) {
    availableScroll =
        max(0., min(available.size() * 20. + 190. - Screen::Height(),
                    availableScroll - dy));
  } else if (dragSide > 0) {
    acceptedScroll =
        max(0., min(accepted.size() * 20. + 160. - Screen::Height(),
                    acceptedScroll - dy));
  } else if (canDrag)
    MapPanel::Drag(dx, dy);
...
bool MissionPanel::Hover(int x, int y) {
  ...
  hoverSort = -1;
  unsigned index =
      max(0, (y + static_cast<int>(availableScroll) - 36 - Screen::Top()) / 20);
  if (x < Screen::Left() + SIDE_WIDTH) {
    if (index < available.size()) {
      dragSide = -1;
      ...
      if (y + static_cast<int>(availableScroll) < Screen::Top() + 30 &&
          y >= Screen::Top() + 10 && x >= Screen::Left() + SIDE_WIDTH - 110) {
...
void MissionPanel::Resize() { ResizeTextArea(); }
...
void MissionPanel::DrawKey() const {
  static const string CONDITIONS[] = {"available", "unavailable", "active",
                                      "blocked", "waypoint"};
  int selected = -1;
  if (availableIt != available.end())
...
void MissionPanel::DrawMissionSystem(const Mission &mission,
                                     const Color &color) const {
  auto toVisit = set<const System *>{mission.Waypoints()};
  toVisit.insert(mission.MarkedSystems().begin(),
                 mission.MarkedSystems().end());
  toVisit.insert(mission.TrackedSystems().begin(),
                 mission.TrackedSystems().end());
  for (const Planet *planet : mission.Stopovers())
    toVisit.insert(planet->GetSystem());
  auto hasVisited = set<const System *>{mission.VisitedWaypoints()};
  hasVisited.insert(mission.UnmarkedSystems().begin(),
                    mission.UnmarkedSystems().end());
  ...
  auto drawRing = [&](const System *system, const Color &drawColor) {
    RingShader::Add(zoom * (system->Position() + center), 22.f, 20.5f,
                    drawColor);
  };
...
// Draw the background for the lists of available and accepted missions (based
// on pos).
Point MissionPanel::DrawPanel(Point pos, const string &label, int entries,
                              bool sorter) const {
  ...
  // Panel sorting
  const Sprite *rush[2] = {SpriteSet::Get("ui/sort rush include"),
                           SpriteSet::Get("ui/sort rush separate")};
  const Sprite *acceptable[2] = {
      SpriteSet::Get("ui/sort unacceptable include"),
      SpriteSet::Get("ui/sort unacceptable separate")};
  ...
  const Sprite *arrow[2] = {SpriteSet::Get("ui/sort descending"),
                            SpriteSet::Get("ui/sort ascending")};

  // Draw Sorting Columns
  if (entries && sorter) {
    SpriteShader::Draw(arrow[player.ShouldSortAscending()],
                       pos + Point(SIDE_WIDTH - 15., 7.5));

    SpriteShader::Draw(sortIcon[player.GetAvailableSortType()],
                       pos + Point(SIDE_WIDTH - 45., 7.5));

    SpriteShader::Draw(acceptable[player.ShouldSortSeparatePossible()],
                       pos + Point(SIDE_WIDTH - 75., 7.5));

    SpriteShader::Draw(rush[player.ShouldSortSeparateDeadline()],
                       pos + Point(SIDE_WIDTH - 105., 7.5));

    if (hoverSort >= 0) {
      Rectangle zone =
          Rectangle(pos + Point(SIDE_WIDTH - 105. + 30 * hoverSort, 7.5),
                    Point(22., 16.));
  ...
  FillShader::Fill(pos + Point(.5 * size.X() - 5., 15.),
                   Point(size.X() - 10., 1.), separatorLine);
...
Point MissionPanel::DrawList(const list<Mission> &missionList, Point pos,
                             const list<Mission>::const_iterator &selectIt,
  ...
  for (auto it = missionList.begin(); it != missionList.end(); ++it) {
    ...
    if (isSelected)
      FillShader::Fill(pos + Point(.5 * SIDE_WIDTH - 5., 8.),
                       Point(SIDE_WIDTH - 10., 20.), highlight);
    ...
    bool canAccept =
        (&missionList == &available ? it->CanAccept(player) : IsSatisfied(*it));
    if (!canAccept) {
    ...
    font.Draw({it->DisplayName(), {SIDE_WIDTH - 11, Truncate::BACK}}, pos,
              *color);
  }
...
void MissionPanel::DrawMissionInfo() {
  ...
  info.SetString("cargo free",
                 Format::SimplePluralization(player.Cargo().Free(), "ton"));
  info.SetString("bunks free", Format::SimplePluralization(
                                   player.Cargo().BunksFree(), "bunk"));
...
void MissionPanel::Accept(bool force) {
  ...
  if (cargoToSell > 0 || crewToFire > 0) {
    ...
    ostringstream out;
    if (cargoToSell > 0 && crewToFire > 0)
      out << "你必须解雇 " << crewToFire << " 名旗舰非必要船员，并出售 "
          << Format::CargoString(cargoToSell, "普通货物")
          << " 才能接受此任务。继续吗？";
    else if (crewToFire > 0)
      out << "你必须解雇 " << crewToFire
          << " 名旗舰非必要船员才能接受此任务。继续吗？";
    else
      out << "你必须出售 " << Format::CargoString(cargoToSell, "普通货物")
          << " 才能接受此任务。继续吗？";
    GetUI().Push(DialogPanel::CallFunctionIfOk(
        this, &MissionPanel::MakeSpaceAndAccept, out.str()));
    return;
  }
  ...
  if (toAccept.Destination()) {
    ...
    // Updates availableIt if matching system found, returns true if planet also
    // matches.
    auto SelectNext = [this, planet, system, &stillLooking](
                          const list<Mission>::const_iterator &it) -> bool {
...
bool MissionPanel::FindMissionForSystem(const System *system) {
  ...
  for (availableIt = available.begin(); availableIt != available.end();
       ++availableIt)
    if (availableIt->Destination()->IsInSystem(system))
      return true;
  for (acceptedIt = accepted.begin(); acceptedIt != accepted.end();
       ++acceptedIt)
    if (acceptedIt->IsVisible() &&
        acceptedIt->Destination()->IsInSystem(system))
      return true;
...
bool MissionPanel::SelectAnyMission() {
  if (availableIt == available.end() && acceptedIt == accepted.end()) {
    // No previous selected mission, so select the first job/mission for any
    // system.
    if (!available.empty())
      availableIt = available.begin();
    else {

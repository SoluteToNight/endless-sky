...
void DrawList(vector<pair<int64_t, string>> &list, const Table &table,
              const string &title, int64_t titleValue, int maxCount = 0,
...
bool CompareModelName(const shared_ptr<Ship> &lhs,
                      const shared_ptr<Ship> &rhs) {
...
bool CompareRequiredCrew(const shared_ptr<Ship> &lhs,
                         const shared_ptr<Ship> &rhs) {
...
InfoPanelState::ShipComparator &
GetReverseCompareFrom(InfoPanelState::ShipComparator &f) {
  if (f == &CompareName)
...
// Table columns and their starting x positions, end x positions, alignment and
// sort comparator.
const PlayerInfoPanel::SortableColumn PlayerInfoPanel::columns[7] = {
    SortableColumn("飞船", 0, 217, {217, Truncate::MIDDLE}, CompareName),
    SortableColumn("型号", 220, 347, {127, Truncate::BACK}, CompareModelName),
    SortableColumn("星系", 350, 487, {137, Truncate::BACK}, CompareSystem),
    SortableColumn("护盾", 550, 493, {57, Alignment::RIGHT, Truncate::BACK},
                   CompareShields),
    SortableColumn("船体", 610, 553, {57, Alignment::RIGHT, Truncate::BACK},
                   CompareHull),
    SortableColumn("燃料", 670, 613, {57, Alignment::RIGHT, Truncate::BACK},
                   CompareFuel),
    SortableColumn("船员", 730, 673, {57, Alignment::RIGHT, Truncate::BACK},
                   CompareRequiredCrew)};
...
PlayerInfoPanel::~PlayerInfoPanel() { Audio::Resume(); }
...
void PlayerInfoPanel::Draw() {
  ...
  if (panelState.CanEdit() && !panelState.Ships().empty()) {
    ...
    const System *flagshipSystem =
        flagship ? flagship->GetSystem() : player.GetSystem();
    ...
    if (hasOtherShips) {
      interfaceInfo.SetCondition(allParked ? "show unpark all"
                                           : "show park all");
      interfaceInfo.SetCondition(allParkedSystem ? "show unpark system"
                                                 : "show park system");
    }
...
bool PlayerInfoPanel::AllowsFastForward() const noexcept { return true; }

bool PlayerInfoPanel::KeyDown(SDL_Keycode key, Uint16 mod,
                              const Command &command, bool isNewPress) {
  ...
  if (key == 'd' || key == SDLK_ESCAPE || (key == 'w' && control) ||
    ...
    if (panelState.AllSelected().empty()) {
      // If no ship was selected, moving up or down selects the first or last
      // ship.
      if (isNewPress) {
        if (key == SDLK_UP)
    ...
    // selected ship group up or down one row.
    else if (panelState.CanEdit() && !panelState.AllSelected().empty() &&
             control && shift) {
      ...
      int selectedIndex =
          panelState.SelectedIndex() + (key == SDLK_DOWN) - (key == SDLK_UP);
      bool isValidIndex =
          static_cast<unsigned>(selectedIndex) < panelState.Ships().size();
      if (selectedIndex < 0) {
        if (isNewPress)
    ...
    if (selected >= 0) {
      if (selected < panelState.Scroll() + LINES_PER_PAGE &&
          selected >= panelState.Scroll()) {
  ...
  } else if (panelState.CanEdit() && (key == 'a') &&
             !panelState.Ships().empty()) {
  ...
  } else if (panelState.CanEdit() && (key == 'c') &&
             !panelState.Ships().empty()) {
...
bool PlayerInfoPanel::Release(int /* x */, int /* y */, MouseButton button) {
  ...
  if (!panelState.CanEdit() || hoverIndex < 0 ||
      hoverIndex == panelState.SelectedIndex())
    return true;
...
void PlayerInfoPanel::DrawPlayer(const Rectangle &bounds) {
  ...
  // Two columns of opposite alignment are used to simulate a single visual
  // column.
  Table table;
  ...
  table.DrawTruncatedPair("驾驶员:", dim,
                          player.FirstName() + " " + player.LastName(), bright,
                          Truncate::MIDDLE, true);
  table.DrawTruncatedPair("净资产:", dim,
                          Format::CreditString(player.Accounts().NetWorth()),
                          bright, Truncate::MIDDLE, true);
  table.DrawTruncatedPair("游戏时长:", dim,
                          Format::PlayTime(player.GetPlayTime()), bright,
                          Truncate::MIDDLE, true);
  ...
  if (!combatRating.empty()) {
    table.DrawGap(10);
    table.DrawUnderline(dim);
    table.Draw("战斗评级:", bright);
    table.Advance();
    table.DrawGap(5);

    table.DrawTruncatedPair("军衔:", dim,
                            to_string(combatLevel) + " - " + combatRating, dim,
                            Truncate::MIDDLE, false);
    table.DrawTruncatedPair("经验:", dim, Format::Number(combatExperience), dim,
                            Truncate::MIDDLE, false);
    bool maxRank =
        (combatRating == GameData::Rating("combat", combatLevel + 1));
    table.DrawTruncatedPair(
        "    距离下一级还需要:", dim,
        maxRank ? "MAX" : Format::Number(ceil(exp(combatLevel + 1))), dim,
        Truncate::MIDDLE, false);
  }
  ...
  string attractionRating =
      GameData::Rating("cargo attractiveness", attractionLevel);
  string deterrenceRating =
      GameData::Rating("armament deterrence", deterrenceLevel);
  if (!attractionRating.empty() && !deterrenceRating.empty()) {
    double attraction =
        max(0., min(1., .005 * (factors.first - factors.second - 2.)));
    double prob = 1. - pow(1. - attraction, 10.);

    table.DrawGap(10);
    table.DrawUnderline(dim);
    table.Draw("海盗威胁度:", bright);
    ...
    table.DrawTruncatedPair("货物: " + attractionRating, dim,
                            "(+" + Format::Number(attractionLevel, 1, false) +
                                ")",
                            dim, Truncate::MIDDLE, false);
    table.DrawTruncatedPair("舰队: " + deterrenceRating, dim,
                            "(-" + Format::Number(deterrenceLevel, 1, false) +
                                ")",
                            dim, Truncate::MIDDLE, false);
  }
  ...
  DrawList(salary, table, "工资:", player.Accounts().SalariesIncomeTotal(), 4);
  ...
  DrawList(tribute, table, "进贡:", player.GetTributeTotal(), 4);
  ...
  DrawList(licenses, table, "执照:", licenses.size(), maxRows, false);
}
...
void PlayerInfoPanel::DrawFleet(const Rectangle &bounds) {
  ...
  for (const auto &column : columns) {
    Rectangle zone =
        Rectangle(tablePoint + Point((column.offset + column.endX) / 2,
                                     table.GetRowSize().Y() / 2),
    ...
    const Color &columnHeaderColor =
        ((!isDragging && zone.Contains(hoverPoint)) ||
  ...
  hoverIndex = -1;
  for (auto sit = panelState.Ships().begin() + panelState.Scroll();
       sit < panelState.Ships().end(); ++sit) {
    ...
    isElsewhere |=
        ((ship.CanBeCarried() || ship.GetPlanet() != player.GetPlanet()) &&
         player.GetPlanet());
    ...
    table.Draw(ship.CanBeCarried() ? "    " + ship.GivenName()
                                   : ship.GivenName());
    ...
    string fuel = to_string(
        static_cast<int>(ship.Attributes().Get("fuel capacity") * ship.Fuel()));
    table.Draw(fuel);
...
void PlayerInfoPanel::SortShips(
    InfoPanelState::ShipComparator *shipComparator) {
  if (panelState.Ships().empty())
    return;
  ...
  multiset<shared_ptr<Ship>, InfoPanelState::ShipComparator *> selectedShips(
      shipComparator);
  ...
  stable_sort(panelState.Ships().begin() + 1, panelState.Ships().end(),
              shipComparator);
...
bool PlayerInfoPanel::Hover(int x, int y) { return Hover(Point(x, y)); }
...
PlayerInfoPanel::SortableColumn::SortableColumn(
    string name, double offset, double endX, Layout layout,
    InfoPanelState::ShipComparator *shipSort)
    : name(name), offset(offset), endX(endX), layout(layout),
      shipSort(shipSort) {}

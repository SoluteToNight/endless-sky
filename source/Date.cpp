...
namespace {
// Convert an integer to a string where single-digit integers have a leading
// zero.
string ZeroPad(int i) { return (i < 10 ? "0" : "") + to_string(i); }
...
const string &Date::ToString() const {
  ...
  if (date && str.empty()) {
    ...
    static const string MONTH[] = {"Jan", "Feb", "Mar", "Apr", "May", "Jun",
    ...
    if (dateFormat == Preferences::DateFormat::YMD)
      str =
          to_string(year) + "年" + ZeroPad(month) + "月" + ZeroPad(day) + "日";
    else if (dateFormat == Preferences::DateFormat::MDY)
      str = to_string(year) + "年" + to_string(month) + "月" + to_string(day) +
            "日";
...
string Date::LongString() const {
  ...
  const string monthStr = to_string(Month());

  return monthStr + "月" + dayString + "日";
}

// Check if this date has been initialized.
Date::operator bool() const { return !!*this; }

// Check if this date has not been initialized.
bool Date::operator!() const { return !date; }
...
// Date comparison.
bool Date::operator<(const Date &other) const { return date < other.date; }
...
int Date::Day() const { return (date & 31); }

// Get the current month (January = 1, rather than being zero-indexed).
int Date::Month() const { return ((date >> 5) & 15); }

// Get the current year.
int Date::Year() const { return (date >> 9); }
...
int Date::WeekdayNumberOffset() const {
  ...
  return (day + (13 * (month + 1)) / 5 + year + year / 4 + 6 * (year / 100) +
          year / 400) %
         7;
}

...
MenuPanel::MenuPanel(PlayerInfo &player, UI &gamePanels)
  ...
  assert(GameData::IsLoaded() &&
         "MenuPanel should only be created after all data is fully loaded");
  ...
  if (mainMenuUi->GetBox("credits").Dimensions()) {
    ...
    // Remove the last 15 lines, as there is already a gap at the beginning of
    // the credits.
    credits.resize(credits.size() - 15);
  } else if (showCreditsWarning) {
    Logger::Log(
        "Interface \"main menu\" does not contain a box for \"credits\".",
...
void MenuPanel::Step() {
  if (Preferences::Has("Animate main menu background")) {
    GameData::StepBackground(
        Point(xSpeed, yAmplitude * sin(animation * TO_RAD)));
    animation += ySpeed;
  } else
  ...
  if (GetUI().IsTop(this) && !scrollingPaused) {
    ...
    if (scroll < 0)
      scroll =
          (20 * static_cast<long long int>(credits.size()) + 299) * SCROLL_MOD;
    if (scroll >=
        (20 * static_cast<long long int>(credits.size()) + 300) * SCROLL_MOD)
      scroll = 0;
  }
}
...
void MenuPanel::Draw() {
  ...
  if (player.IsLoaded() && !player.IsDead()) {
    ...
    info.SetString("credits",
                   Format::AbbreviatedNumber(player.Accounts().Credits()));
...
bool MenuPanel::KeyDown(SDL_Keycode key, Uint16 mod, const Command &command,
  ...
  if (player.IsLoaded() && (key == 'e' || command.Has(Command::MENU))) {
    ...
    GetUI().Push(new StartConditionsPanel(player, gamePanels,
                                          GameData::StartOptions(), nullptr));
  } else if (key == 'g' && player.IsLoaded() && !player.IsDead() &&
             !player.GetGamerules().LockGamerules())

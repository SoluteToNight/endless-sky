...
LoadPanel::LoadPanel(PlayerInfo &player, UI &gamePanels)
    : player(player), gamePanels(gamePanels),
      ...
      tooltip(200, Alignment::LEFT, Tooltip::Direction::DOWN_LEFT,
  ...
  if (player.GetPlanet() && !player.IsDead() &&
      !gamePanels.IsTop(&*gamePanels.Root()) && gamePanels.CanSave())
    player.Save();
  UpdateLists();
}
...
void LoadPanel::Draw() {
  ...
  // Draw the list of pilots.
  {
    ...
    for (const auto &it : files) {
      ...
      const Point textPoint(drawPoint.X() + hTextPad,
                            zone.Center().Y() - font.Height() / 2);
      bool isHighlighted = (it.first == selectedPilot ||
                            (hasHover && zone.Contains(hoverPoint)));
  ...
  if (!selectedPilot.empty() && files.contains(selectedPilot)) {
    ...
    for (const auto &it : files.find(selectedPilot)->second) {
      ...
      Rectangle zone(drawPoint + Point(snapshotBox.Width() / 2., 10.),
                     Point(snapshotBox.Width(), 20.));
      const Point textPoint(drawPoint.X() + hTextPad,
                            zone.Center().Y() - font.Height() / 2);
      ...
      font.Draw({name, {textWidth, Truncate::BACK}}, textPoint,
                Color((isHighlighted ? .7 : .5) * alpha, 0.));
    }
  }
...
void LoadPanel::UpdateTooltipActivation() { tooltip.UpdateActivationCount(); }
...
bool LoadPanel::KeyDown(SDL_Keycode key, Uint16 mod, const Command &command,
  ...
  if (key == 'n') {
    ...
    GetUI().Push(DialogPanel::RequestString(
        ...
        "为此快照输入一个名称，或使用最近存档的日期:", FileDate(lastSave)));
...
bool LoadPanel::Drag(double dx, double dy) {
  ...
  if (sideHasFocus)
    ...
    centerScroll =
        max(0., min(20. * it->second.size() - 280., centerScroll - dy));
  return true;
}
...
void LoadPanel::UpdateLists() {
  ...
  for (const auto &path : fileList) {
    ...
    // The file name is either "Pilot Name.txt" or "Pilot
    // Name~SnapshotTitle.txt".
    ...
    // Ensure that the main save for this pilot, not a snapshot, is first in the
    // list.
    if (!isSnapshot)
  ...
  for (auto &it : files) {
    // Don't include the first item in the sort if this pilot has a non-snapshot
    // save.
    auto start = it.second.begin();
    if (start->first.find('~') == string::npos)
      ++start;
    sort(start, it.second.end(),
         [](const pair<string, filesystem::file_time_type> &a,
            const pair<string, filesystem::file_time_type> &b) -> bool {
...
void LoadPanel::WriteSnapshot(const filesystem::path &sourceFile,
                              const filesystem::path &snapshotName) {
  ...
  if (Files::Copy(sourceFile, snapshotName)) {
    ...
    GetUI().Push(DialogPanel::Info("错误: 无法创建文件 \"" +
                                   snapshotName.string() + "\"。"));
}
...
void LoadPanel::LoadCallback() {
  ...
  // Scale any new masks that might have been added by the newly loaded save
  // file.
...
void LoadPanel::DeletePilot(const string &) {
  ...
  if (failed)
    GetUI().Push(DialogPanel::Info("删除驾驶员文件失败。"));
...
void LoadPanel::DeleteSave() {
  ...
  if (Files::Exists(path))
    GetUI().Push(DialogPanel::Info("删除快照文件失败。"));

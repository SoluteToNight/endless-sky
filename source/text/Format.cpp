...
string MLAShorthand(int64_t value) {
  ...
  for (size_t magnitude = 0; magnitude < WORD_NUMBERS.size() - 1; ++magnitude) {
    ...
    char buf[BUFLEN] = {0};
    snprintf(buf, BUFLEN, "%s%.3f %s", negative ? "negative " : "",
             above / 1000.0, WORD_NUMBERS[magnitude].first);
    buf[BUFLEN - 1] = '\0';
    return string(buf);
  }
  return string();
...
// Format the decimal places of a double value, up to a given number of decimal
// places, and with trailing zeros removed if desired. The given double is
// expected to already be the fractional part of a number (i.e. something less
// than 0).
void FormatDecimals(double &value, double fraction, int places,
                    bool trimTrailingZeros, string &result) {
  if (!places || (!fraction && trimTrailingZeros))
    return;
  // Keep track of the digit in each place.
  double digit;
  vector<int> digits(places);
  // Account for floating-point representation error by adding EPS after
  // multiplying.
  constexpr double EPS = 1e-10;
  for (int i = 0; i < places; ++i) {
    fraction = modf(fraction * 10. + EPS, &digit);
    // If this digit is about equal to 10, then carry the 1 to the digit prior
    // to this one.
    if (digit >= 10 - EPS) {
      for (int j = i - 1; j >= -1; --j) {
        // If this digit is the 10ths place, then 1 needs to be added to the
        // value preceding the decimal place.
        if (j == -1)
          ++value;
        else {
          ++digits[j];
...
string
StringSubstituter(const string &source,
...
void ExpandInto(const string &key, const string &oldValue,
                const map<string, string> &source, map<string, string> &result,
...
//
// source.substr(formatStart, formatSize) contains the format (credits, mass,
// etc.) source.substr(conditionStart, conditionSize) contains the condition
// name
//
...
void AppendCondition(string &result, const string &source,
                     const Format::ConditionGetter &getter, size_t formatStart,
                     size_t formatSize, size_t conditionStart,
  ...
  if (formatStart == string::npos || formatSize == string::npos)
    ...
    result.append(
        Format::ChicagoForm(value, true)); // Thirty-three or One hundred one
  else if (IsFormat("mla"))
    result.append(Format::MLAForm(value, false)); // thirty-three or 101
  else if (IsFormat("Mla"))
    result.append(
        Format::MLAForm(value, true)); // Thirty-three or One hundred one
  else if (IsFormat("words"))
    result.append(
        Format::WordForm(value, false)); // thirty-three or one hundred one
  else if (IsFormat("Words"))
    result.append(
        Format::WordForm(value, true)); // Thirty-three or One hundred one
  else
...
string Format::AbbreviatedNumber(int64_t value) {
  ...
  static constexpr array<pair<int64_t, char>, 3> THRESHOLD_SUFFIX = {
      {{1000000000000ll, 'T'}, {1000000000ll, 'B'}, {1000000ll, 'M'}}};
...
string Format::AbbreviatedNumber(double value, optional<int> decimalPlaces,
...
// Convert the given number into abbreviated format as described in
// Format::AbbreviatedNumber, then attach the ' credit' or ' credits' suffix to
// it.
...
string Format::CargoString(double amount, const string &cargo) {
  return MassString(amount) + " " + cargo;
}
...
string Format::PlayTime(double timeVal) {
  ...
  // Break time into larger and larger units until the largest one, or the value
  // is empty
  size_t i = 0;
  do {
    int period =
        (i < SUFFIX.size() - 1 ? timeValFormat % PERIOD[i] : timeValFormat);
...
string Format::TimestampString(chrono::time_point<chrono::system_clock> time,
  ...
  const char *format =
      ignorePreferences ? DEFAULT_TIMESTAMP_FORMAT : TimestampFormatString();
...
string Format::TimestampString(filesystem::file_time_type time) {
  auto sctp = time_point_cast<chrono::system_clock::duration>(
      time - filesystem::file_time_type::clock::now() +
...
string Format::Number(double value, optional<int> decimalPlaces,
                      bool trimTrailingZeros) {
  if (!value) {
    string result = "0";
    FormatDecimals(value, value, decimalPlaces.value_or(0), trimTrailingZeros,
                   result);
    return result;
  }
  if (std::isnan(value))
  ...
  // If no explicit decimal places were given, then only show at least one
  // decimal place for numbers with a magnitude less than 10,000. Values less
  // than 1,000 may have two decimal places.
  int places =
      decimalPlaces.value_or(value >= 10000 ? 0 : (value >= 1000 ? 1 : 2));
  if (places) {
...
string Format::Percentage(double value, optional<int> decimalPlaces,
                          bool trimTrailingZeros) {
...
string Format::WordForm(int64_t value, bool startOfSentence) {
  ...
  if (!result.empty() && startOfSentence && result[0] >= 'a' &&
      result[0] <= 'z')
    result[0] -= 32;
  return result;
}
...
void Format::ReplaceAll(string &text, const string &target,
...
string Format::ExpandConditions(const string &source,
                                const ConditionGetter &getter) {
  ...
  //	state = @ ----- read &[...@ but haven't seen ] yet. Have format start &
  //size.

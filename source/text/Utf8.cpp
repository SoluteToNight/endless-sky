...
wstring ToUTF16(const string &input, bool isPath) {
  ...
  int size = MultiByteToWideChar(page, 0, &input[0],
                                 input.length() - endsInSlash, nullptr, 0);
  result.resize(size);
  MultiByteToWideChar(page, 0, &input[0], input.length() - endsInSlash,
                      &result[0], size);
...
string ToUTF8(const wchar_t *str) {
  ...
  int size =
      WideCharToMultiByte(page, 0, str, -1, nullptr, 0, nullptr, nullptr) - 1;
  result.resize(size);
...
bool IsBOM(char32_t c) { return c == BOM; }
...
// Advance the given position to the start of the next character.

...
public:
  // Calculate and check the most number of pointer positions that should be
  // available for active missions. This can be up to half the maximum number of
  // pointers that can be drawn.
  void Reserve();
...
void PointerDrawCount::Reserve() {
  maximumActive =
      max(MapPanel::MAX_MISSION_POINTERS_DRAWN / 2,
          MapPanel::MAX_MISSION_POINTERS_DRAWN - (available + unavailable));
}

unsigned PointerDrawCount::MaximumActive() const { return maximumActive; }
...
void TallyEscorts(const vector<shared_ptr<Ship>> &escorts,
  ...
  for (const auto &ship : escorts) {
    ...
    // If this ship has no system but has a parent, it is carried (and thus not
    // parked).
    else if (ship->CanBeCarried() && ship->GetParent() &&
             ship->GetParent()->GetSystem())
...
bool HasMultipleLandablePlanets(const System &system) {
  ...
  for (auto &&stellarObject : system.Objects())
    if (stellarObject.HasValidPlanet() && stellarObject.HasSprite() &&
        !stellarObject.GetPlanet()->IsWormhole()) {
...
// Return total value of raid fleet (if any) and 60 frames worth of system
// danger.
double DangerFleetTotal(const PlayerInfo &player, const System &system,
                        const bool withRaids) {
...
void MapPanel::DrawPointer(Point position, unsigned &systemCount,
                           const Color &color, bool drawBack, bool bigger) {
  if (++systemCount > MAX_MISSION_POINTERS_DRAWN)
    return;
  Angle angle = Angle(MISSION_POINTERS_ANGLE_DELTA * systemCount);
  if (drawBack)
    PointerShader::Draw(position, angle.Unit(), 14.f + bigger,
                        19.f + 2 * bigger, -4.f, black);
  PointerShader::Draw(position, angle.Unit(), 8.f + bigger, 15.f + 2 * bigger,
                      -6.f, color);
}

pair<bool, bool> MapPanel::BlinkMissionIndicator(const PlayerInfo &player,
...
MapPanel::MapPanel(PlayerInfo &player, int commodity, const System *special,
                   bool fromMission)
    : player(player), distance(player), playerSystem(*player.GetSystem()),
      selectedSystem(special ? special : player.GetSystem()),
      specialSystem(special),
      playerJumpDistance(System::DEFAULT_NEIGHBOR_DISTANCE),
      commodity(commodity),
      tooltip(170, Alignment::LEFT, Tooltip::Direction::DOWN_RIGHT,
              Tooltip::Corner::BOTTOM_RIGHT,
  ...
  if (Preferences::Has(SHOW_ESCORT_SYSTEMS) ||
      Preferences::Has(SHOW_STORED_OUTFITS)) {
    escortSystems.clear();
    if (Preferences::Has(SHOW_ESCORT_SYSTEMS))
  ...
  double playerRange =
      player.Flagship() ? player.Flagship()->JumpNavigation().JumpRange() : 0.;
...
MapPanel::~MapPanel() { Audio::Resume(); }
...
void MapPanel::Step() {
  ...
  // The mouse should be pointing to the same map position before and after
  // zooming.
...
void MapPanel::Draw() {
  ...
  for (const auto &it : GameData::Galaxies())
    SpriteShader::Draw(it.second.GetSprite(),
                       Zoom() * (center + it.second.Position()), Zoom());
  ...
  RingShader::Draw(Zoom() * (playerSystem.Position() + center),
                   System::DEFAULT_NEIGHBOR_DISTANCE * Zoom(), 2.0f, 1.0f,
                   viewRangeColor);
  // Draw the jump range circle around your current location if it is different
  ...
  if (playerJumpDistance != System::DEFAULT_NEIGHBOR_DISTANCE)
    RingShader::Draw(Zoom() * (playerSystem.Position() + center),
                     (playerJumpDistance + .5) * Zoom(),
                     (playerJumpDistance - .5) * Zoom(), jumpRangeColor);

  // Draw a circle around the selected system.
  Color brightColor(.4f, 0.f);
  RingShader::Draw(Zoom() * (selectedSystem->Position() + center), 11.f, 9.f,
                   brightColor);
...
void MapPanel::FinishDrawing(const string &buttonCondition) {
  ...
  const Interface *mapButtonUi = GameData::Interfaces().Get(
      Screen::Width() < 1300 ? "map buttons (small screen)" : "map buttons");
  ...
  if (hoverSystem && tooltip.ShouldDraw()) {
    ...
    if (!tooltip.HasText()) {
      ...
      if (t.activeShips && t.parkedShips)
        text += to_string(t.activeShips) +
                (t.activeShips == 1 ? " 艘活跃护卫舰\n" : " 艘活跃护卫舰\n");
      else if (t.activeShips)
        text += to_string(t.activeShips) + " 艘护卫舰";
      if (t.parkedShips)
        text += to_string(t.parkedShips) + " 艘停泊的护卫舰";
      if (!t.outfits.empty()) {
        if (t.activeShips || t.parkedShips)
          text += "\n";
        ...
        text += to_string(sum) + " 件存储装备";

        if (HasMultipleLandablePlanets(*hoverSystem) || t.outfits.size() > 1)
          for (const auto &it : t.outfits)
            text += "\n - " + to_string(it.second) + " 件在 " +
                    it.first->DisplayName();
      }

      tooltip.SetText(text);
    }
    tooltip.SetZone((hoverSystem->Position() + center) * Zoom(),
                    Point(20., 20.));
    tooltip.Draw();
  }
  ...
  if (selectedSystem != &playerSystem && !distance.HasRoute(*selectedSystem)) {
    static const string NO_SHIP = "你没有旗舰可以跳跃！";
    static const string NO_DRIVE = "你没有安装跳跃引擎！";
    static const string UNAVAILABLE = "你没有可用的航线到达该星系。";
    static const string UNKNOWN = "你尚未绘制到达该星系的航线。";
...
bool MapPanel::AllowsFastForward() const noexcept { return true; }

void MapPanel::UpdateTooltipActivation() { tooltip.UpdateActivationCount(); }

bool MapPanel::KeyDown(SDL_Keycode key, Uint16 mod, const Command &command,
                       bool isNewPress) {
  // When changing the map mode, explicitly close all child panels (for example,
  ...
  if (command.Has(Command::MAP) || key == 'd' || key == SDLK_ESCAPE ||
    ...
    GetUI().Push(DialogPanel::RequestString(this, &MapPanel::Find, "搜索:", "",
                                            Truncate::NONE, true));
    return true;
...
bool MapPanel::Click(int x, int y, MouseButton button, int clicks) {
  ...
  for (const auto &it : GameData::Systems()) {
    ...
    if (system.IsValid() && !system.Inaccessible() &&
        click.Distance(system.Position()) < 10. &&
...
// If the mouse has moved near a known system that contains escorts, track the
// dwell time.
...
Color MapPanel::MapColor(double value) {
  ...
  if (value < 0.)
    return Color(.12 + .12 * value, .48 + .36 * value, .48 - .12 * value, .4);
  else
    return Color(.12 + .48 * value, .48, .48 - .48 * value, .4);
}

Color MapPanel::ReputationColor(double reputation, bool canLand,
                                bool hasDominated) {
...
Color MapPanel::GovernmentColor(const Government *government) {
  ...
  return Color(.6f * government->GetColor().Get()[0],
               .6f * government->GetColor().Get()[1],
               .6f * government->GetColor().Get()[2], .4f);
}
...
double MapPanel::SystemValue(const System *system) const { return 0.; }
...
void MapPanel::Select(const System *system) {
  ...
  // Update the cache to apply any visual changes needed after the selected
  // system was changed.
  UpdateCache();
  ...
  const System *source =
      isJumping ? flagship->GetTargetSystem() : &playerSystem;

  auto mod = SDL_GetModState();
  // TODO: Whoever called Select should tell us what to do with this system
  // vis-a-vis the travel plan, rather than possibly manipulating it both here
  // and there. Or, we entirely separate Select from travel plan modifications.
  ...
  // Reset the travel destination if the final system in the travel plan has
  // changed.
  if (!plan.empty())
...
double MapPanel::Zoom() const { return pow(1.5, zoom.AnimatedValue()); }
...
bool MapPanel::GetTravelInfo(const System *previous, const System *next,
                             const double jumpRange, bool &isJump,
                             bool &isWormhole, bool &isMappable,
                             Color *wormholeColor) const {
  const bool isHyper = previous->Links().contains(next);
  isWormhole = false;
  isMappable = false;
  // Short-circuit the loop for MissionPanel, which draws hyperlinks and
  // wormholes the same.
  if (!isHyper || wormholeColor)
    for (const StellarObject &object : previous->Objects())
      if (object.HasSprite() && object.HasValidPlanet() &&
          object.GetPlanet()->IsWormhole() &&
          player.HasVisited(*object.GetPlanet()) && player.CanView(*previous) &&
          player.CanView(*next) &&
          &object.GetPlanet()->GetWormhole()->WormholeDestination(*previous) ==
              next) {
        isWormhole = true;
  ...
  isJump = !isHyper && !isWormhole &&
           previous->JumpNeighbors(jumpRange).contains(next);
...
void MapPanel::UpdateCache() {
  ...
  if (commodity == SHOW_DANGER) {
    // Scale danger to span [0, 1] based on known systems, without including
    // raid fleets as those can greatly skew the range once they start having a
    // chance of appearing, leading to silly (and not very useful) displays.
  ...
  for (const auto &it : GameData::Systems()) {
    const System &system = it.second;
    // Ignore systems which are inaccessible or have been referred to, but not
    // actually defined.
    if (!system.IsValid() || system.Inaccessible())
      continue;
    // Ignore systems the player has never seen, unless they have a pending
    // mission that lets them see it.
    ...
    if (!player.CanView(system))
      color = UnexploredColor();
             ...
             commodity == SHOW_SPECIAL || commodity == SHOW_VISITED ||
             commodity == SHOW_DANGER) {
        ...
        for (const StellarObject &object : system.Objects())
          if (object.HasSprite() && object.HasValidPlanet()) {
            ...
            if (planet->IsWormhole() ||
                !planet->IsAccessible(player.Flagship()))
              continue;
    ...
    static const vector<const Sprite *> unmappedSystem = {
        SpriteSet::Get("map/unexplored-star")};
    ...
    nodes.emplace_back(system.Position(), color,
                       ...
                       (&system == &playerSystem || &system == selectedSystem)
                           ? closeNameColor
                           : farNameColor,
  ...
  // The link color depends on whether it's connected to the current system or
  // not.
...
void MapPanel::DrawTravelPlan() {
  ...
  for (const shared_ptr<Ship> &it : player.Ships())
    if (!it->IsParked() && (!it->CanBeCarried() || it.get() == flagship) &&
  ...
  for (int i = player.TravelPlan().size() - 1; i >= 0; --i, previous = next) {
    ...
    Color wormholeColor;
    if (!GetTravelInfo(previous, next, jumpRange, isJump, isWormhole,
                       isMappable, &wormholeColor))
      break;
    ...
    if (!isWormhole)
      for (auto &it : fuel)
        if (it.second >= 0.) {
          double cost = it.first->JumpNavigation()
                            .GetCheapestJumpType(previous, next)
                            .second;
...
void MapPanel::DrawSelectedSystem() {
  ...
  SpriteShader::Draw(sprite, Point(0. + selectedSystemOffset,
                                   Screen::Top() + .5f * sprite->Height()));

  string text;
  if (!player.KnowsName(*selectedSystem))
    text = "已选星系：未知星系";
  else
    text = "已选星系：" + selectedSystem->DisplayName();
  ...
  Point pos(-175. + selectedSystemOffset,
            Screen::Top() + .5 * (30. - font.Height()));
  font.Draw({text, {350, Alignment::CENTER, Truncate::MIDDLE}}, pos,
            *GameData::Colors().Get("bright"));

  // Reset the position of this UI element. If something is in the way, it will
  // be moved back before it's drawn the next frame.
  ...
  for (const auto &squad : escortSystems)
    if (player.HasSeen(*squad.first) || squad.first == specialSystem) {
      ...
      if (squad.second.activeShips || squad.second.parkedShips)
        RingShader::Draw(pos, INNER - 1.f, 0.f,
                         squad.second.activeShips ? active : parked);

      if (!squad.second.outfits.empty())
        // Stored outfits are drawn/indicated by 8 short rays out of the system
        // center.
        for (int i = 0; i < 8; ++i) {
          static constexpr float WIDTH = 1.6f;

          // Starting at 7.5 degrees to intentionally mis-align with mission
          // pointers.
...
void MapPanel::DrawWormholes() {
  ...
  // A system can host more than one set of wormholes (e.g. Cardea), and some
  // wormholes may even share a link vector.
  for (auto &&it : GameData::Wormholes()) {
    ...
    for (auto &&link : it.second.Links())
      if (!link.first->Inaccessible() && !link.second->Inaccessible() &&
          p.IsInSystem(link.first) && player.CanView(*link.first) &&
          player.CanView(*link.second))
        arrowsToDraw.emplace_back(link.first, link.second,
                                  it.second.GetLinkColor());
  }
  ...
  for (const WormholeArrow &link : arrowsToDraw) {
    ...
    if (link.from < link.to || !count_if(
                                   arrowsToDraw.begin(), arrowsToDraw.end(),
                                   [&link](const WormholeArrow &cmp) {
                                     return cmp.from == link.to &&
                                            cmp.to == link.from;
                                   }))
...
void MapPanel::DrawSystems() {
  ...
  for (const Node &node : nodes) {
    ...
    if (commodity != SHOW_STARS)
      ...
      // Ensures every multiple-star system has a characteristic, deterministic
      // rotation.
      Angle starAngle = 0;
      ...
      for (int i = 0; i < starsToDraw; ++i) {
        ...
        const Body starBody(star, pos + zoom * starOffset * starAngle.Unit(),
                            Point(0, 0), 0, cbrt(zoom) * 0.6, Point(1., 1.),
                            0.8);
        starBatch.Add(starBody);
      }
    }

    if (commodity == SHOW_GOVERNMENT && node.government &&
        node.government->DisplayName() != "Uninhabited") {
...
void MapPanel::DrawNames() {
  ...
  double alpha =
      zoom >= BIGGER_TARGET_ZOOM ? 1. : (.5 - zoom) / (.5 - BIGGER_TARGET_ZOOM);
  ...
  for (const Node &node : nodes)
    font.Draw(node.name, zoom * (node.position + center) + offset,
              node.nameColor.Transparent(alpha));
}
...
void MapPanel::DrawMissions() {
  ...
  if (specialSystem) {
    ...
    Angle a =
        Angle(MISSION_POINTERS_ANGLE_DELTA * missionCount[specialSystem].drawn);
  ...
  for_each(missionCount.begin(), missionCount.end(),
           [](auto &it) { it.second.Reserve(); });
  for (const Mission &mission : player.Missions()) {
    ...
    if (it.drawn < it.MaximumActive()) {
      ...
      const Color &color = blink.first   ? black
    ...
    for (const System *waypoint : mission.Waypoints())
      DrawPointer(waypoint, missionCount[waypoint].drawn,
                  missionCount[waypoint].MaximumActive(), waypointColor);
    for (const Planet *stopover : mission.Stopovers()) {
      const System *stopoverSystem = stopover->GetSystem();
      auto &counts = missionCount[stopoverSystem];
      DrawPointer(stopoverSystem, counts.drawn, counts.MaximumActive(),
                  waypointColor);
    }
    for (const System *mark : mission.MarkedSystems())
      DrawPointer(mark, missionCount[mark].drawn,
                  missionCount[mark].MaximumActive(), waypointColor);
  ...
  for (auto &&it : missionCount) {
    ...
    for (unsigned i = 0; i < counters.available; ++i)
      DrawPointer(system, counters.drawn, MAX_MISSION_POINTERS_DRAWN,
                  availableColor);
...
void MapPanel::DrawPointer(const System *system, unsigned &systemCount,
                           unsigned max, const Color &color, bool bigger) {
  if (systemCount >= max)
    return;
  DrawPointer(Zoom() * (system->Position() + center), systemCount, color, true,
              bigger);
}
...
void MapPanel::IncrementZoom() {
  const Interface *mapInterface = GameData::Interfaces().Get("map");
  double newZoom =
      min<double>(mapInterface->GetValue("max zoom"), player.MapZoom() + 1);
...
void MapPanel::DecrementZoom() {
  const Interface *mapInterface = GameData::Interfaces().Get("map");
  double newZoom =
      max<double>(mapInterface->GetValue("min zoom"), player.MapZoom() - 1);
  zoom.Set(newZoom, mapInterface->GetValue("zoom animation duration"));
  player.SetMapZoom(newZoom);
}

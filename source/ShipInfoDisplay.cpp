...
ShipInfoDisplay::ShipInfoDisplay(const Ship &ship, const PlayerInfo &player,
                                 bool descriptionCollapsed) {
  Update(ship, player, descriptionCollapsed);
}

// Call this every time the ship changes.
// Panels that have scrolling abilities are not limited by space, allowing more
// detailed attributes.
void ShipInfoDisplay::Update(const Ship &ship, const PlayerInfo &player,
  ...
  const Depreciation &depreciation =
      ship.IsYours() ? player.FleetDepreciation() : player.StockDepreciation();
...
int ShipInfoDisplay::OutfitsHeight() const { return outfitsHeight; }
...
void ShipInfoDisplay::DrawAttributes(const Point &topLeft,
                                     const bool sale) const {
  // Header.
  ...
  if (sale) {
    ...
    FillShader::Fill(point + Point(.5 * WIDTH, 5.), Point(WIDTH - 20., 1.),
                     color);
  } else
    point -= Point(0, 10.);
...
void ShipInfoDisplay::UpdateAttributes(const Ship &ship,
  ...
  if (!ship.IsYours())
    for (const string &license : attributes.Licenses()) {
      ...
      const auto &licenseOutfit =
          GameData::Outfits().Find(license + " License");
  ...
  const Depreciation &depreciation =
      ship.IsYours() ? player.FleetDepreciation() : player.StockDepreciation();
  int64_t depreciated =
      depreciation.Value(ship, player.GetDate().DaysSinceEpoch());
  ...
  attributeValues.push_back(
      Format::Number(isGeneric ? emptyMass : currentMass) + " tons");
  attributesHeight += 20;
  ...
  attributeValues.push_back(string());
  attributesHeight += 10;
  attributeLabels.push_back(isGeneric ? "movement (full - no cargo):"
                                      : "movement:");
  attributeValues.push_back(string());
  attributesHeight += 20;
  ...
  double baseAccel =
      3600. * forwardThrust * (1. + attributes.Get("acceleration multiplier"));
  if (!isGeneric)
  ...
  double baseTurn =
      60. * attributes.Get("turn") * (1. + attributes.Get("turn multiplier"));
  if (!isGeneric)
  ...
  for (const auto &category : GameData::GetCategory(CategoryType::BAY)) {
    ...
    if (totalBays) {
      ...
      string bayLabel = Format::LowerCase(bayType);
  ...
  const double idleEnergyPerFrame =
  ...
  const double idleHeatPerFrame =
      attributes.Get("heat generation") + attributes.Get("solar heat") +
      attributes.Get("fuel heat") -
      ship.CoolingEfficiency() *
  ...
  const double movingEnergyPerFrame =
      max(attributes.Get("thrusting energy"),
  ...
  const double movingHeatPerFrame =
      max(attributes.Get("thrusting heat"),
  ...
  attributesHeight += 20;
  double shieldEnergy =
      (hasShieldRegen) ? (attributes.Get("shield energy") +
  ...
  if (scrollingPanel) {
    ...
    attributesHeight += 20;
    const double overallEnergy = idleEnergyPerFrame - movingEnergyPerFrame -
                                 firingEnergy - shieldEnergy - hullEnergy;
    const double overallHeat = idleHeatPerFrame + movingHeatPerFrame +
                               firingHeat + shieldHeat + hullHeat;
...
void ShipInfoDisplay::UpdateOutfits(const Ship &ship, const PlayerInfo &player,
                                    const Depreciation &depreciation) {
  outfitLabels.clear();
  outfitValues.clear();
  outfitsHeight = 20;
  ...
  for (const auto &it : ship.Outfits())
    if (it.first->IsDefined() && !it.first->Category().empty() &&
        !it.first->DisplayName().empty())
  ...
  int64_t totalCost =
      depreciation.Value(ship, player.GetDate().DaysSinceEpoch());
  int64_t chassisCost =
      depreciation.Value(GameData::Ships().Get(ship.TrueModelName()),
